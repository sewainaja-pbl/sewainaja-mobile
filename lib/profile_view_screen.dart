...
class _ProfileViewScreenState extends State<ProfileViewScreen> {
  ...
  void initState() {
    ...
    print(
      '[ProfileViewScreen] initState: widget.ownerId = "${widget.ownerId}", _targetOwnerId = "$_targetOwnerId"',
    );
    _loadProfile();
    _checkFollowStatus();
  ...
  Future<void> _loadProfile() async {
    ...
    print(
      '[ProfileViewScreen] _loadProfile: fetching profile for "$_targetOwnerId"',
    );
  ...
  Widget build(BuildContext context) {
    ...
    final bool isOwnProfile =
        FirebaseAuth.instance.currentUser?.uid == _targetOwnerId;
    final double ratingNum =
        (_userProfile?['avgRatingAsOwner'] as num?)?.toDouble() ??
        double.tryParse(widget.rating ?? '') ??
        0.0;
    final String displayRating = ratingNum > 0
    ...
    final String statsListings = widget.listingCount != null
        ? "${widget.listingCount} Barang Aktif"
        : "$totalTransactions Transaksi";
    final String aboutMeText =
        _userProfile?['bio'] ?? "Belum ada deskripsi profil.";
                                          ...
                                          child: ElevatedButton(
                                            onPressed: () async {
                                              ...
                                              if (currentUserId == null) {
                                                showAppErrorSnack(
                                                  context,
                                                  "Silakan login untuk follow user.",
                                                );
                                                return;
                                              }
                                              if (currentUserId ==
                                                ...
                                                showAppErrorSnack(
                                                  context,
                                                  "Anda tidak dapat mem-follow diri sendiri.",
                                                );
                                                return;
                                              }
                                  ...
                                  showProductMoreSheet(
                                    ...
                                    onReportPressed: () {
                                      ...
                                      if (reportedId.isEmpty) {
                                        showAppErrorSnack(
                                          context,
                                          'Informasi pemilik tidak tersedia untuk dilaporkan.',
                                        );
                                        return;
                                      }
                                      ...
                                      if (currentUserId == reportedId) {
                                        showAppErrorSnack(
                                          context,
                                          'Anda tidak bisa melaporkan barang Anda sendiri.',
                                        );
                                        return;
                                      }
                                    ...
                                    onDeletePressed: isOwnItem
                                        ? () async {
                                            ...
                                            try {
                                              final token =
                                                  await const AuthSessionService()
                                                      .getValidIdToken();
                                              final response = await http.delete(
                                                Uri.parse(
                                                  '${ApiConfig.baseUrl}/items/${itemModel.id}',
                                                ),
                                                headers: {
                                                  'Authorization':
                                                      'Bearer $token',
